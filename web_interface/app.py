...
function bulkReadStatus() {
    console.log("Starte Bulk-Read..."); // Debugging: Bulk-Read gestartet
    fetch('/bulk_read', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
    }).then(response => response.json()).then(data => {
        console.log("Antwort vom Bulk-Read:", data); // Debugging: Antwort des Servers
        if (data.status === "success") {
    ...
    }).catch(error => console.error('Fehler beim Bulk-Read:', error));
}
